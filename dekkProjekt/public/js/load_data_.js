...
class GeoJSON {
    constructor(data, id) {
        let me = this;
        this.id = id;
        this.maxValue = d3.max(data['features'], o => o.properties.value);
        this.geojson = L.geoJson(data, { // dataset1
            style: function(feature) { return me.styling(feature, id) },
    ...
    styling(feature, dataset) {
        return {
            weight: 2,
            opacity: 1,
            color: '#007472',
            fillOpacity: 0.5,
            fillColor: getColor(feature.properties.value, dataset)
        };
    }

    resetHighlight(e) {
        if (map.hasLayer(this.geojson)) {
...
function drawMap() {
        ...
        geojson1 = L.geoJson(districts1, { // dataset1
            style: function(feature) { return styling(feature, 'dataset1') },
            onEachFeature: onEachFeature
        });

        geojson2 = L.geoJson(districts2, { // dataset2
            style: function(feature) { return styling(feature, 'dataset2') },
            onEachFeature: onEachFeature
        });

        geojson3 = new GeoJSON(districts1, 'dataset3');
...
// style districts
function styling(feature, dataset) {
    return {
        weight: 2,
        opacity: 1,
        color: '#007472',
